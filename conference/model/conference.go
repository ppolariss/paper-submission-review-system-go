...
type Conference struct {
	...
	ConferenceStartAt      time.Time `json:"conference_start_at" gorm:"column:conference start_at"`
	ConferenceEndAt        time.Time `json:"conference_end_at" gorm:"column:conference end_at"`
	Venue                  string    `json:"venue" gorm:"length:100"`
	SubmissionDeadline     time.Time `json:"submission_deadline"`
	ReviewResultAnnounceAt time.Time `json:"reviewResult_announce_at" gorm:"column:review_result announce_at"`
...
type ConferenceApplication struct {
	ID                int       `json:"id" gorm:"primary_key autoIncrement column:applicationid"`
